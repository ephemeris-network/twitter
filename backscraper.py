...
"""the database used"""
database = table.test_backdata

"""defining the time period over which to scrape"""
filename = './backscrape_data.json'

"""the scrape period"""
n = 1 #to be multiplied by the scrape period, to define how far to go back on each scrape 
...
try: 
    ...
    while 1: 
        ...
        for i, handle in enumerate(handles):
            ...
            search_from =  str((datetime.utcnow() - timedelta(days=days_back)).date())
            search_until = str((datetime.utcnow() - timedelta(days=days_back) + timedelta(days=scrape_period)).date())
